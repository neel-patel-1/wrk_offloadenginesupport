...
do
	if [ "${1}" = "tlso" ]; then 
		./wrk -t16 -c1024 -e qatengine -d${2}s https://192.168.1.2:443/file${i}.txt > $wrk_output/offload_${2}_${i}.wrk
	elif [ "${1}" = "tls" ]; then 
		./wrk -t16 -c1024 -d${2}s https://192.168.1.2:443/file${i}.txt > $wrk_output/tls_${2}_${i}.wrk
	else
		./wrk -t16 -c1024 -d${2}s http://192.168.1.2:80/file${i}.txt > $wrk_output/tcp_${2}_${i}.wrk
	fi
done
